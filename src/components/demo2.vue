...
let keduListB: null | number[] = [] // （大）刻度的数组
let keduListS: null | number[] = [] // （小）刻度的数组
const getKeduListB = (): number[] => {
  const arr = []
...
let radiusScaleS: any = null // 刻度线性比例尺
let rotate_standard: null | number = null
let rotate_current: null | number = null
let value_current: null | number = null
const draw = (el: HTMLDivElement) => {
  ...
  group
    ...
    .attr(
      'd',
      arc({ startAngle: radiusScaleB(0), endAngle: radiusScaleB(1) } as d3.DefaultArcObject),
    )
    .attr('fill', '#f2f2f2')
    .attr('stroke', '#cccccc')
    .attr('stroke-width', 3)
  ...
  function dragEnd(event: DragEvent) {
    emits('update:modelValue', value_current!)
  }
  ...
  group
    ...
    .attr('fill', '#f2f2f2')
    .attr('stroke', '#cccccc')
    .call(d3.drag().on('drag', dragged).on('end', dragEnd) as any)
